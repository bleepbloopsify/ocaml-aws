...
let of_http body =
  try
    ...
    try
      Util.or_error
        (Util.option_bind resp GetBucketAnalyticsConfigurationOutput.parse)
        (let open Error in
           BadResponse
             {
               body;
               message =
                 "Could not find well formed GetBucketAnalyticsConfigurationOutput."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing GetBucketAnalyticsConfigurationOutput - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
